...
function fOnLoad(){
  ...
  FRMListado.fSetTitulo(fCheckBoxTodos("cbTodos", "fTodos")+",Ejercicio,Núm. Sol.,Lim.Ent.Docs.,Estim. Entrega,Trámite,");
  FRMListado.fSetObjs(0,"Caja");
//  FRMListado.fSetObjs(6,"Boton");
  //FRMListado.fSetObjs(6,"Boton");
  //FRMListado.fSetObjs(7,"Boton");
  //FRMListado.fSetCampos("0,1,8,9,11,15,");
  FRMListado.fSetCampos("0,1,8,9,15,");
...
//  FRMListado.fSetDespliega("text,text,text,text,text,logico,text,text,text,");
  FRMListado.fSetAlinea("center,center,center,center,center,left,");
  FRMListado.fSetDespliega("text,text,text,text,text,text,");
  ...
  fDisabled(true,"iEjercicioBusqueda,iNumSolicitudBusqueda,");
...
function fNavega(){
  ...
  if (frm.iCveSolicitante.value != ""){
    ...
    if(frm.lVenUni.value == true){//si es ventanilla unica de cualquier entidad solo puede traer las de esa entidad 
    	frm.hdFiltro.value += " AND RS.iCveOficina= "+frm.hdCveOfic.value;
    	frm.hdFiltro.value += " AND RS.ICVEDEPARTAMENTO= "+frm.hdCveDpto.value;
    	frm.hdFiltro.value += " AND RS.LRESOLUCIONPOSITIVA = 0";
    }
    ...
    if(lVenUni == true){//si es ventanilla unica de cualquier entidad solo puede traer las de esa entidad
    	frm.hdFiltro.value += " AND RS.iCveOficina= "+frm.hdCveOfic.value;
    	frm.hdFiltro.value += " AND RS.ICVEDEPARTAMENTO= "+frm.hdCveDpto.value;
    	frm.hdFiltro.value += " AND RS.LRESOLUCIONPOSITIVA = 0";
    }
...
function fResultado(aRes,cId,cError,cNavStatus,iRowPag,cLlave){
  if(cError!=""){
    ...
    if("CIDOficinaDeptoXUsr"){
    	frm.hdCveOfic.value=aRes[0][1];
    	frm.hdCveDpto.value=aRes[0][2];
    	if(frm.hdCveDpto.value == 95) // si es ventanilla unica solo puede traer las solicitudes que le corresponden
    		lVenUni = true;
    }
  }
}
...
//                       "    AND ET.ICVEETAPA in (17,21) " +
                       "    AND ET.ICVEETAPA in (8) " +
                       "  LEFT JOIN GRLDOMICILIO D ON D.ICVEPERSONA = RS.ICVEREPLEGAL " +
                       "    AND D.ICVEDOMICILIO = RS.ICVEDOMICILIOREPLEGAL " +
                       " LEFT JOIN GRLPAIS ON GRLPAIS.ICVEPAIS = D.ICVEPAIS " +
                       "  LEFT JOIN GRLENTIDADFED ON GRLENTIDADFED.ICVEPAIS = D.ICVEPAIS " +
                       "    AND GRLENTIDADFED.ICVEENTIDADFED = D.ICVEENTIDADFED " +
                       " LEFT JOIN GRLMUNICIPIO ON GRLMUNICIPIO.ICVEPAIS = D.ICVEPAIS " +
                       "    AND GRLMUNICIPIO.ICVEENTIDADFED = D.ICVEENTIDADFED " +
                       "    AND GRLMUNICIPIO.ICVEMUNICIPIO = D.ICVEMUNICIPIO " +
                       "  LEFT JOIN GRLLOCALIDAD ON D.ICVEPAIS = GRLLOCALIDAD.ICVEPAIS " +
                       ...
                       "  LEFT JOIN GRLTIPODOMICILIO ON GRLTIPODOMICILIO.ICVETIPODOMICILIO = D.ICVETIPODOMICILIO " +
                       ...
                       " left JOIN GRLLOCALIDAD LOCP ON DP.ICVEPAIS = LOCP.ICVEPAIS " +
                      ...
                      if(lVenUni==true){
		                      frm.hdSelect.value += " AND RS.iCveOficina= "+frm.hdCveOfic.value;
		                      frm.hdSelect.value += " AND RS.ICVEDEPARTAMENTO= "+frm.hdCveDpto.value;
		                      frm.hdSelect.value += " AND RS.LRESOLUCIONPOSITIVA = 0 ";
                      }
                       
                   	 frm.hdSelect.value+= "  ORDER BY IEJERCICIO, INUMSOLICITUD ";
                       /*"UNION " +
                       "SELECT " +
                       "       RS.IEJERCICIO, " +
                       "       RS.INUMSOLICITUD, " +
