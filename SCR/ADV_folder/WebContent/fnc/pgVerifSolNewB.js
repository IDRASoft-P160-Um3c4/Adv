...
function fBefLoad(){
  ...
  cTitulo = (cTitulo == "" || cTitulo == "TÍTULO NO ENCONTRADO")?"EVALUACIÓN DE REQUISITOS JURÍDICOS Y LEGALES":cTitulo;
  fSetWindowTitle();
}
...
function fResultado(aRes,cId,cError,cNavStatus,iRowPag,cLlave,iEtapaVerifica,iEtapaRecepcion,irPNC,iNotificacion,iVentanillaU,iModifTram){
	...
	if(cId == "Listado01A" && cError==""){
	 ...
	 if(cId == "cIdTerminoArea" && cError==""){ 	
		   fAlert("\nHa finalizado la evaluación de requisitos por parte de la DGDC---DAJL para ésta solicitud.");
	 }
