...
function fBefLoad() {
	cPaginaWebJS = "pgVerifSolNewB.js";
	if (top.fGetTituloPagina)
		cTitulo = "EVALUACIÓN DE DOCUMENTOS JURÍDICOS Y LEGALES";
	fSetWindowTitle();
}
...
function fDefPag() {
	...
	TDEtiCampo(false, "EEtiqueta", 0, " Número de Solicitud :",
			"iNumSolicitud", "", 30, 30, " Solicitud ", "fMayus(this);");
	FTR();
	FinTabla();
	FITR();
	TDEtiAreaTexto(false, "EEtiqueta", 0, " Trámite:", 90, 2, "cDscTramite",
			"", " Trámite", "fMayus(this);", "", "", true, true, false, "", 0);
	FITR();
	TDEtiCampo(false, "EEtiqueta", 0, " Modalidad:", "cDscModalidad", "", 90,
			90, " Modalidad", "fMayus(this);");
	FTR();
	...
	FTR();
	ITR("", 0, "", "175", "center", "top");
	TDEtiCheckBox("EEtiqueta", 0, "Requisito Válido:", "lValidoBOX", "1", true,
			" Activo", "", "");
	FTR();
	FinTabla();
	FTD();
...
function fNavega() {
	...
	if (frm.iNumSolicitud.value != "" && frm.iNumSolicitud.value.length > 0
		...
		var modif = " and ICONSECUTIVOPNC is NOT null)"; // null or
															// ICONSECUTIVOPNC =
															// 0))"; //con
															// iconsecutivo = 0
															// ya no trae los
		...
		frm.hdFiltro.value = " TRAREGSOLICITUD.iEjercicio = "
				...
				+ " and TRAREQUISITO.ICVEOFICINAEVAL in (SELECT ICVEOFICINA from GRLUSUARIOXOFIC where ICVEUSUARIO ="
				+ frm.iCveUs.value
				+ ")"
				+ " and TRAREQUISITO.ICVEDEPTOEVAL in (SELECT ICVEDEPARTAMENTO from GRLUSUARIOXOFIC where ICVEUSUARIO ="
				+ frm.iCveUs.value + ")";
...
function fResultado(aRes, cId, cError, cNavStatus, iRowPag, cLlave,
		iEtapaVerifica, iEtapaRecepcion, irPNC, iNotificacion, iVentanillaU,
		iModifTram) {
	...
	if (cId == "cIdTerminoArea" && cError == "") {
		fAlert("\nHa finalizado la evaluación de requisitos por parte de la DAJL para ésta solicitud.");
		fTerminaEvaluacion();
	}
...
function fGuardar() {
	...
	if (frm.iCveRequisito.value == "") {
		...
		return false;
	} else if (frm.lValidoBOX.checked == true && frm.iCveRequisito.value != ""
...
function fGuardarProcesa() {
	...
	if (faltaCotejo != true) {
		...
		if (fValidaTodo() == true && frm.lValidoBOX.checked == false
				&& dtEvaluacion == "") {
			frm.hdBoton.value = "Guardar";
...
function fTerminaEvaluacion() {
	if (top.opener.fTerminaEvaluacion)
		top.opener.fTerminaEvaluacion();
	fSalir();
}
...
function fSoloAlfanumericos2(cVCadena) {
	if (fRaros(cVCadena) || fPuntuacion(cVCadena) || fComa(cVCadena)
			|| fSignos(cVCadena) || fArroba(cVCadena) || fParentesis(cVCadena))
		return false;
	else
		return true;
}
...
function fTerminadoArea() {
	...
	if (frm.iNumSolicitud.value != "" && frm.iNumSolicitud.value.length > 0
			&& frm.iEjercicio.value != "" && frm.iEjercicio.value.length > 0) {
		var modif = " and ICONSECUTIVOPNC is NOT null)";// or ICONSECUTIVOPNC =
														// 0))"; //con
														// iconsecutivo = 0 ya
														// no trae los que
