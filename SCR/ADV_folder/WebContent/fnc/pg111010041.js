        ...
        InicioTabla("ETablaInfo",0,"95%","","","",1);
          ...
          ITR();
           ...
           FITR();
              ...
              TDEtiSelect(false,"EEtiqueta",0,"","iCveFormato","");
              TDEtiCheckBox("EEtiqueta",0,"","lDigitalizaBOX","1",true," Vigente");
              Hidden("lDigitaliza","");
              
              ITRTD("ETablaST",6,"","","center");
              	TextoSimple("DEPARTAMENTO EVALUADOR");
              FTDTR();
          
              FITR(); 
                 TDEtiSelect(true,"EEtiqueta",0,"Oficina:","iCveOficinaEval","fOficinaOnChange();");  //COMBOS PARA SELECCIONAR LA OFICINA Y DPTO QUE EVALUARA
              FITR(); 
                 TDEtiSelect(true,"EEtiqueta",0,"Departamento:","iCveDeptoEval","");				  //EL REQUISITO
           FITR(); 

//=========================================================================
//FCSReq1 -Se Agrego el Campo cMetodoEjecuta y Etiqueta "Metodo Ejecutar" -
           FITR();
             TDEtiCampo(false,"EEtiqueta",0,"","cMetodoEjecuta","",80,80," Metodo ejecutar","fMayus(this);","","",false,"",5);
           FITR();
...
function fResultado(aRes,cId,cError,cNavStatus,iRowPag,cLlave){
  ...
  if(cId == "Listado" && cError==""){
   ...
   if(cId == "CIDOficina" && cError==""){
	    fFillSelect(frm.iCveOficinaEval,aRes,false,frm.iCveOficinaEval.value,0,1);
	    fOficinaOnChange();
	  }
  if(cId == "CIDDepartamento" && cError==""){
    fFillSelect(frm.iCveDeptoEval,aRes,false,frm.iCveDeptoEval.value,0,1);
  }
}
...
function fNuevo(){
  ...
  frm.hdNumReg.value =  100;
   ...
   frm.hdLlave.value ="";
   frm.hdSelect.value = "SELECT O.ICVEOFICINA, O.CDSCBREVE FROM GRLOFICINA O WHERE O.CDSCBREVE <> '(NO ESPECIFICADO)' ORDER BY CDSCOFICINA";   
   
   if(fEngSubmite("pgConsulta.jsp","CIDOficina")==true){
	   FRMPanel.fSetTraStatus("UpdateBegin");
	   fBlanked();
	   fDisabled(false,"iCveRequisito,","--");
	   FRMListado.fSetDisabled(true);
...
function fSelReg(aDato){
	...
	//alert(aDato[9]+"|"+aDato[10]+"---------"+aDato[11]+"|"+aDato[12]);
...
//FCSReq1 -Se Agrego para obtener el valor de cMetodoEjecuta y Mostrarlo en Pantalla -
   ...
   fAsignaSelect(frm.iCveOficinaEval,aDato[9],aDato[10]);
   fAsignaSelect(frm.iCveDeptoEval,aDato[11],aDato[12]);
...
function fOficinaOnChange(){
	  ...
	  frm.hdSelect.value = "Select grldepartamento.iCveDepartamento,cDscBreve from GRLDepartamento " +
	                      "  join GRLDEPTOXOFIC on GRLDEPTOXOFIC.ICVEDEPARTAMENTO = GRLDEPARTAMENTO.ICVEDEPARTAMENTO Where iCveOficina = " + frm.iCveOficinaEval.value 
	                      + " and cDscDepartamento <> '(NO ESPECIFICADO)' Order By cDscDepartamento";
	  fEngSubmite("pgConsulta.jsp","CIDDepartamento");
	}


function fOcultaCamposRequisitos(){
	frm.cMetodoEjecuta.style.visibility="hidden";
	frm.iCveFormato.style.visibility="hidden";
	frm.lDigitalizaBOX.style.visibility="hidden";
}