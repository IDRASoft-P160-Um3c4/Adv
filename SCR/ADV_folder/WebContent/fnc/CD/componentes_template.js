...
function BtnImg(cNombreM,cNomImgM,cHRefM,cEstatusM,l4Status,cImgIni){ // 3000-boton de tipo imagen
   iImgs++;
...
function Text(lMandatorioM,cNombreM,cValorM,iLargoM,iMaxCaracteresM,cToolTip,cOnBlur,cOnAnyEvent,cOnChange,lSelectOnFocus,lActivo){ // 3000-Campo de Texto
   ...
   return fInput("text", cNombreM, '"'+cValorM+'"', iLargoM, iMaxCaracteresM, cToolTip, cOnBlur, cOnAnyEvent, cOnChange, lSelectOnFocus, lActivo)
}
...
function Liga(cNombreM,cHRefM,cEstatusM){ // 3000-Boton de tipo imagen
   cTx='<a href="JavaScript:'+cHRefM+'"'+"\n";
...
function CheckBox(cNombreM,cValorM,lSeleccion,cToolTip,cOnBlur,cOnAnyEvent,cOnChange,lSelectOnFocus,lActivo){ // 3000-Caja de Seleccion
...
function Select(cNombreM,cOnChange){ // 3000-Lista Desplegable simple
...
function SelectList(cNombreM,iSizeM,cOnChange){ // 3000-Lista simple
...
function Estilo(cNombreM,cDefineM){ // 3000-Definicion de algun estilo de Texto de forma local
   cTx='<style>'+cNombreM+'{'+cDefineM+'}</style>';
   cGPD+=cTx+"\n";
   return cTx;
}
function Marquee(cMsgM,cClassM,cAlignM,cDelayM,cDireccionM,cAnchoM,cAltoM){ // 3000-Generacion de un Marquee
...
//4000 Tablas
function InicioTabla(cEstiloM,iBordeM,cAncho,cAlto,cAlign,cImgFondo,iEspacioCel,iRellenoCel,cBGColor){ // 4000-Genera la especificacion de una tabla dinamica
...
function DinTabla(cIDM,cEstiloM,iBordeM,cAncho,cAlto,cAlign,cValign,cImgFondo,iEspacioCel,iRellenoCel,cBGColor){ // 4000-Genera la especificacion de una tabla
   cTx='<table border="'+iBordeM+'"';
   if(cIDM){
     if(cIDM != "")
       cTx+=' ID="'+cIDM+'"';
...
function ITR(cEstilo,cAncho,cAlto,cAlign,cValign,iRenExtiende,cAnyEvent){ // 4000-Genera la definicion de un nuevo renglon de una tabla
   cTx='<tr';
   if(cEstilo){
     if(cEstilo != "")
       cTx+=' class="'+cEstilo+'"';
...
function ITD(cEstilo,iColExtiende,cAncho,cAlto,cAlign,cValign){ // 4000-Genera la definicion de una columna
   cTx='<td';
   if(cEstilo){
     if(cEstilo != "")
       cTx+=' class="'+cEstilo+'"';
...
function ITRTD(cEstilo,iColExtiende,cAncho,cAlto,cAlign,cValign){ // 4000-Genera la definicion de un renglon y una columna
...
function FITD(cEstilo,iColExtiende,cAncho,cAlto,cAlign,cValign){ // 4000-Genera la definicion del fin de una columna y el inicio de Otra
   cTx = FTD();
   cTx = ITD(cEstilo,iColExtiende,cAncho,cAlto,cAlign,cValign);
   return cTx;
}
function FTDTR(){ // 4000-Genera la finalizacion de una columna y de un renglon de una tabla
   cTx="</td></tr>";
   cGPD+=cTx+"\n";
   return cTx;
}
function FITR(){ // 4000-Genera la finalizacion del renglon de una tabla e inserta uno nuevo
   cTx="</tr><tr>";
   cGPD+=cTx+"\n";
   return cTx;
}
function FTR(){ // 4000-Genera la finalizacion del renglon de una tabla
   cTx="</tr>";
   cGPD+=cTx+"\n";
   return cTx;
}
function FinTabla(){ // 4000-Genera la finalizacion de una tabla
   cTx="</table>";
   cGPD+=cTx+"\n";
   return cTx;
}
...
function fLoadCal(oObjM){// 3000-Llama al Calendario
  if(oObjM.disabled == false){
    ...
    fAbreSubWindow(true,"CD/calendario","no","no","no","no",200,200,375,290);
  }
}
