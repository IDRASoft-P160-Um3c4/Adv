...
function fBefLoad(){
  ...
  cTitulo = (cTitulo == "" || cTitulo == "TÍTULO NO ENCONTRADO")?"RATIFICACIÓN FINAL DE LA D.G.D.C.":cTitulo;
  fSetWindowTitle();
    if(top.opener){
...
//                          TextoSimple("Oficio y Notificación de PNC");
//                       FTD();
//                    FTR();
//                    ITR();
//                       TDEtiCampo(false,"EEtiqueta",0," Fecha de Oficio:","dtOficio","",10,10," Notificación","fMayus(this);");
////                       TDEtiCampo(false,"EEtiqueta",0," Num. Oficio:","cNumOficio","",20,20," Notificación","fMayus(this);");
//                    FTR();
//                    
//                   
//
...
function fResultado(aRes,cId,cError,cNavStatus,iRowPag,cLlave,iEtapaVerifica,iEtapaRecepcion,irPNC,iNotificacion,iVentanillaU,iModifTram){
  ...
  if(cId == "idCambiaEtapa" && cError==""){
    ...
    if(noValidos==true && faltaEvaluacion==false){
      if(confirm("Existe uno ó más requisitos no válidos. Se generará un Producto No Conforme. ¿Desea continuar?")){
...
function fBefLoad(){
  ...
  if(top.fGetTituloPagina)
     cTitulo = top.fGetTituloPagina(cPaginaWebJS).toUpperCase();
  cTitulo = (cTitulo == "" || cTitulo == "TÍTULO NO ENCONTRADO")?"EVALUACIÓN TÉCNICA":cTitulo;
  fSetWindowTitle();
    if(top.opener){
//    cPaginaWebJS = "pgSolicitud.js";
     if(top.opener.cPaginaWebJS == "pgSolicitud.js")
           ...
           ITR();
              ITD();
                 InicioTabla("",1,"100%","top","center");
                    ...
                    FTR();
                    ITR();
                       //TDEtiCheckBox("EEtiqueta",0,"Requisito Válido:","lValidoBOX","1",true," Activo","","onClick=fHabDeshab();");
...
// LLAMADO desde el Panel cada vez que se presiona al botón Guardar
function fGuardar(){
    if(frm.lValidoBOX.checked== true && frm.dtRecepcion.value == ""){
	fAlert("No es posible aprobar un requisito que no ha sido entregado.");
	return false;
    }
     if(frm.cNumOficio.value != "" && frm.lValidoBOX.checked == true){
       if(!lTienePNC){
         frm.hdBoton.value = "VelidaDespuesNotif";
         fNavega();
       }else fAlert("El registro no puede ser validado mientras tenga un PNC");
     }
     else{
       if(lEnVerificacion == false){
         if(frm.dtNotificacion.value == ""){
