 ...
 * @version 1.0
 */
public class TDTRARegReqXTram extends DAOBase{
...
public TVDinRep updateTienePNC(TVDinRep vData,Connection cnNested) throws
 ...
 try{
   ...
   return vData;
	...
	String lSQL =
	       "insert into TRARegReqXTram(iEjercicio,iNumSolicitud,iCveTramite,iCveModalidad,iCveRequisito,dtRecepcion,iCveUsuRecibe,lfisico) values (?,?,?,?,?,?,?,?)";
	 ...
	 Vector vcRequisitosDigitales= this.findByCustom(""," SELECT ICVEREQUISITO FROM TRAREQDIGINT WHERE IEJERCICIO =  " +vData.getInt("iEjercicio")+ " AND INUMSOLICITUD = "+vData.getInt("iNumSolicitud"));
	 ...
	 for(i = 0;i < vcRequisitos.size();i++){
		 ...
		 for(int j=0 ; j < vcRequisitosDigitales.size();j++){
			 TVDinRep vIcveReqDig = (TVDinRep) vcRequisitosDigitales.get(j); 
			 
			 if(vIcveReqDig.getInt("ICVEREQUISITO") == vReq.getInt("ICVEREQUISITO")){
				 lfisico=0;
				 break;
			 }
		 }
