...
public class UploadADVDocs extends HttpServlet {
		...
		try {
			...
			while (iterValid.hasNext()) {
				...
				if(!item.isFormField()&&item.getSize()>maxSizeFile*1024 * 1024){
					...
					loadPag(VParametros, response, "El tama駉 del archivo \""+nombre+"\" es mayor a "+maxSizeFile+"mb. Revise el documento e intente nuevamente.");
					return;					
				}
			}
	...
	private void loadPag(TParametro VParametros, HttpServletResponse response, String cError){
		...
		if(cPag!=null && !cPag.equals(""))
			...
			pagRet+="pg111020016ADV.js\"></SCRIPT>\r\n";
