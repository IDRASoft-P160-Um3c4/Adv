...
/**
 * <p>Title: TDTRARegRefPagoA1.java</p>
 ...
 * @version 1.0
 */
public class TDTRARegRefPagoA1 extends DAOBase{
   ...
   * <p><b> insert into TRARegRefPago(iEjercicio,iNumSolicitud,iConsecutivo,iRefNumerica,cRefAlfaNum,dImportePagar,dImportePagado,dtPago,iCveBanco,cFormatoSAT) values (?,?,?,?,?,?,?,?,?,?) </b></p>
  ...
  public TVDinRep insert(TVDinRep vData,Connection cnNested) throws
    ...
    try{
      ...
      String lSQL =
          "insert into TRARegRefPago(iEjercicio,iNumSolicitud,iConsecutivo,iRefNumerica,cRefAlfaNum,dImportePagar,dImportePagado,dtPago,iCveBanco,cFormatoSAT,lPagado) values (?,?,?,?,?,?,?,?,?,?,?)";

      //AGREGAR AL ULTIMO ...
      Vector vcData = findByCustom("","select MAX(iEjercicio) AS iEjercicio from TRARegRefPago");
      if(vcData.size() > 0){
      ...
      lPStmt.setString(4,vData.getString("RefNumerica"));
   ...
   * <p><b> update TRARegRefPago set iRefNumerica=?, cRefAlfaNum=?, dImportePagar=?, dImportePagado=?, dtPago=?, iCveBanco=?, cFormatoSAT=? where iEjercicio = ? AND iNumSolicitud = ? AND iConsecutivo = ?  </b></p>
   * <p><b> Campos Llave: iEjercicio,iNumSolicitud,iConsecutivo, </b></p>
   * @param vData TVDinRep      - VO Dinámico que contiene a la entidad a Insertar.
   * @param cnNested Connection - Conexión anidada que permite que el método se encuentre dentro de una transacción mayor.
   * @throws DAOException       - Excepción de tipo DAO
  ...
  public TVDinRep update(TVDinRep vData,Connection cnNested) throws
    ...
    try{
      ...
      String lSQL = "update TRARegRefPago set iRefNumerica=?, cRefAlfaNum=?, dImportePagar=?, dImportePagado=?, dtPago=?, iCveBanco=?, cFormatoSAT=?, lPagado=? where iEjercicio = ? AND iNumSolicitud = ? AND iConsecutivo = ? ";
      ...
      lPStmt.setFloat(3,vData.getFloat("dImportePagar"));
      lPStmt.setFloat(4,vData.getFloat("dImportePagado"));
      ...
      lPStmt.setInt(10,vData.getInt("iNumSolicitud"));
      lPStmt.setInt(11,vData.getInt("iConsecutivo"));
      lPStmt.executeUpdate();
      if(cnNested == null){
        conn.commit();
      }
