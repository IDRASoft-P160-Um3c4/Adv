 ...
 */
  ...
  if(!request.getParameter("hdEtapa").equalsIgnoreCase("null") && !request.getParameter("hdEtapa").equalsIgnoreCase(""))
    iCveEtapa = vParametros.getPropEspecifica(request.getParameter("hdEtapa"));
  else
    cError = "Los datos no fueron recibidos correctamente, favor de efectuar la búsqueda nuevamente.";

  if(!request.getParameter("hdEtapaGuardar").equalsIgnoreCase("null") && !request.getParameter("hdEtapaGuardar").equalsIgnoreCase(""))
    iCveEtapaGuardar = Integer.parseInt(vParametros.getPropEspecifica(request.getParameter("hdEtapaGuardar")));
  else
    cError = "Los datos no fueron recibidos correctamente, favor de efectuar la búsqueda nuevamente.";
  ...
  if(oAccion.getCAccion().equals("RecibeTramite")){
    vDinRep = oAccion.setInputs("iEjercicio,iNumSolicitud,iCveTramite,iCveModalidad,lAnexo,iCveOficinaUsr,iCveDeptoUsr,hdCveUsuario");

    try{
      if(iCveEtapaGuardar != 0){
        vDinRep.put("iCveEtapa",iCveEtapaGuardar);
        vDinRep.put("iEtapaOficialia",iCveEtapa);
        if(iCveEtapaGuardar == Integer.parseInt(vParametros.getPropEspecifica("EtapaRegistro")))
          vDinRep.put("iCveDepartamentoAsg",request.getParameter("iCveDepartamentoAsg"));

        vDinRep = dTRARecepcion.recepcionTramite(vDinRep,null);
      }else
        cError = "Guardar";
    }catch(Exception e){
      cError="Guardar";
  ...
  if(oAccion.getCAccion().equals("RecibeTramiteInt")){
	  System.out.println("Dentro del jsp recibe trámite internet...");
	    vDinRep = oAccion.setInputs("iEjercicio,iNumSolicitud,iCveTramite,iCveModalidad,lAnexo,iCveOficinaUsr,iCveDeptoUsr,hdCveUsuario");
	    ...
	    try{
	      if(iCveEtapaGuardar != 0){
	        ...
	        System.out.println("Antes de hacer la integracion de la fecha de integracion...");
	        intSol.updateFechaIntegra(vDinRep.getInt("iEjercicio"),vDinRep.getInt("iNumSolicitud"),vDinRep.getInt("iCveDeptoUsr"),vDinRep.getInt("hdCveUsuario"),null);
	        vDinRep = dTRARecepcion.recepcionTramite(vDinRep,null);
	      }else
	        cError = "Guardar";
	    }catch(Exception e){
...
sb.append("AND TRAREGETAPASXMODTRAM.iCveOficina = " + request.getParameter("iCveOficinaUsr"));
sb.append(" AND TRAREGETAPASXMODTRAM.iCveDepartamento = " + request.getParameter("iCveDeptoUsr"));
sb.append(" AND TRARegTramXSol.dtCancelacion IS NULL ");
sb.append(" AND TRAREGETAPASXMODTRAM.ICVEETAPA in ( " + iCveEtapa+")");
...
String cFiltro = oAccion.getCFiltro().toUpperCase().replaceAll("WHERE"," AND ");
  Vector vcListado = dTRARecepcion.findByCustom("iEjercicio,iNumSolicitud",sb.toString() + cFiltro + oAccion.getCOrden());
