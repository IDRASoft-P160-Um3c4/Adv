// MetaCD=1.0
 ...
 // Description: Consulta de Solicitudes por Estatus
 // Company: Infotec
 ...
 function fOnLoad(){
   ...
   frm.hdOrden.value = "cDscBreveOfic";
 ...
 function fResultado(aRes,cId,cError,cNavStatus,iRowPag,cLlave){
   ...
   if(cId == "CIDOficinaDeptoXUsrTodas" && cError == ""){
     fFillSelect(frm.iCveDeptoUsr,aRes,true,frm.iCveDeptoUsr.value,0,2);
   }
...
function fDeptoUsrOnChange(){
  frm.hdSelect.value = "SELECT DISTINCT(T.iCveTramite),T.CDSCBREVE FROM  TRATRAMITEXOFIC TXO " +
                       "JOIN TRACATTRAMITE T ON T.ICVETRAMITE=TXO.ICVETRAMITE " +
                       "  WHERE ICVEOFICINARESUELVE="+frm.iCveOficinaUsr.value+
                       "    AND ICVEDEPTORESUELVE="+frm.iCveDeptoUsr.value+
                       "    AND T.LVIGENTE=1 " +
                       "ORDER BY T.CDSCBREVE ";
  frm.hdLlave.value  = "iCveTramite";
  frm.hdFiltro.value = "ICVEOFICINARESUELVE="+frm.iCveOficinaUsr.value+" AND ICVEDEPTORESUELVE="+frm.iCveDeptoUsr.value;
  frm.hdOrden.value  = "";
  frm.hdNumReg.value = 10000;
  fEngSubmite("pgConsulta.jsp","cIdTramites");
}
