...
var cBGColorGrid1 = '699971';//'699971';
  var cBGColorSelList = '699971';//'BEF781';
function fBefLoad(){
...
function fSetColTit(iColM,cAlineaciónM,cTamaño){
  aColTit[iColTit]=[iColM,cAlineaciónM,cTamaño];
  iColTit++;
}
function fSetCol(iColM,cAlineaciónM){
  aCols[iCols]=[iColM,cAlineaciónM];
  iCols++;
}
  ...
  if (aTextosLogicos.length != 3){
   ...
   if(aListado.length > 0){
     ...
     for(i=0;i<aListado.length;i++){
        ...
        if((i%2) == 0)
          newRow.style.backgroundColor=cBGColorGrid1;
        h = 0;j = 0;
        if(cCual==''){
          lKeep=true;
          while(lKeep==true){
            ...
            for(r=0;r<aOCL.length;r++){ //Objetos Columna
               ...
               if(aTOCL[0] == h){
                 ...
                 newCell.className = "EEtiquetaLST1";
