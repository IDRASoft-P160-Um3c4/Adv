...
function fDefPag(){
 ...
 function fNavega(){
   ...
   frm.hdBoton.value = "MultiplesSol";
 ...
 function fResultado(aRes,cId,cError,cNavStatus,iRowPag,cLlave){
  ...
  if(cId == "cIdPagIngresos" && cError==""){
   ...
   if(cId == "cIdUsrYPwd" && cError==""){
     ...
     //frm.hdPassUsrIngresos.value = "3011";
...
function fCalcula(){
  ...
  aCValoresText = FRMListado.fGetObjs(2);
   ...
   //Al principio de listado siempre se van a poner los grupos, osea si hay grupos van a aparecer cuadros de texto
  ...
  for(cont=0;cont < aCBoxCalcular.length;cont++){
    if(aCBoxCalcular[cont]){
      ...
      if (parseInt(aResultadoTemp[cont][0],10)>0){ //Si es un grupo
        ...
        if(frm.cDscConcepto.value == "") frm.cDscConcepto.value = "-"; else frm.cDscConcepto.value += "¨-";
...
//    alert("Toma valores de la persona\nTramite :"+iCveTramite+"\nModalidad: "+iCveModalidad);
...
function fObtieneDatosOpener(){
   if(top.opener){
     ...
     if(top.opener.fGetASolMarcadas)
       aSolicitudes = top.opener.fGetASolMarcadas();
