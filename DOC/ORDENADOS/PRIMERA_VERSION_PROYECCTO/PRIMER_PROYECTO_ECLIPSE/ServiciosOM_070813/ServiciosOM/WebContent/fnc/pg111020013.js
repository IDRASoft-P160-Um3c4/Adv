    ...
    ITRTD("",0,"","1","center");
      InicioTabla("ETablaInfo",0,"95%","","","",1);
        ITRTD("ETablaST",6,"","","center");
          ...
          BtnImgNombre("vgbuscar","lupa","if(fNavega(true));","Actualizar solicitudes del solicitante", false, "", "BuscarSolicitudes");
        FTDTR();
        ITR();
          TDEtiCheckBox("EEtiqueta",0,"Mostrar Impresas:","lImpresasBOX","0",true," Activo","","");
            ...
            TDEtiCampo(false,"EEtiqueta",0,"Número de Solicitud:","iNumSolicitudBusca","",10,10,"Número de Solicitud","","","","","ECampo",0);
         //  TDEtiCheckBox("EEtiqueta",0,"Mostrar Impresas:","lImpresasBOX","0",true," Activo","","onClick=fNavega();");
        FTR();
        ITR();
          TDEtiCampo(true,"EEtiqueta",0," Solicitante:","Persona_cNomRazonSocial","",50,50," Solicitante","fMayus(this);","","",false,"EEtiquetaL",0);
          TDEtiCampo(true,"EEtiqueta",0," Rep. Legal:","RepLegal_cNomRazonSocial","",50,50," Representante Legal","fMayus(this);","","",false,"EEtiquetaL",3);
        FITR();
          ...
          TDEtiAreaTexto(false,"EEtiqueta",0,"Dom. Rep. Legal:",49,3,"RepLegal_cDscDomicilio","","Domicilio","fMayus(this);","","",false,false,false,"EEtiquetaL",3);
        FITR();
          TDEtiCampo(false,"EEtiqueta",0," R.F.C.:","cRFC","",14,13," R.F.C.","fMayus(this);","","",false,"EEtiquetaL",0);
          TDEtiAreaTexto(false,"EEtiqueta \" rowspan=\"2",0,"Observación:",49,4,"cObsTramite","","Observaciones","fMayus(this);","","",false,false,false,"EEtiquetaL\" rowspan=\"3",3);
        FITR();
...
function fOnLoad(){
  ...
  FRMListado.fSetTitulo(fCheckBoxTodos("cbTodos", "fTodos")+",Ejercicio,Núm. Sol.,Lim.Ent.Docs.,Estim. Entrega,Principal,Conceptos,Pagar,Trámite,Oficina de Origen,");
  FRMListado.fSetObjs(0,"Caja");
  FRMListado.fSetObjs(6,"Boton");
  FRMListado.fSetObjs(7,"Boton");
  FRMListado.fSetCampos("0,1,8,9,11,15,21,");
...
function fNavega(lMensaje){
  frm.hdFiltro.value = "";
   if(frm.iCveSolicitante.value != "")
     frm.hdFiltro.value =  " RS.iCveSolicitante = " + frm.iCveSolicitante.value + " ";
   if(frm.iEjercicioSol.value != ""){
     ...
     if(frm.iNumSolicitudBusca.value != "")
        frm.hdFiltro.value += " AND RS.INUMSOLICITUD = " + frm.iNumSolicitudBusca.value + " ";
   }
   frm.hdFiltro.value += ((frm.hdFiltro.value != "")?" AND ":" ");
   frm.hdFiltro.value += " RS.lImpreso = " + ((frm.lImpresasBOX.checked)?"1":"0") + " ";
   frm.hdOrden.value =  FRMFiltro.fGetOrden();
   frm.hdNumReg.value =  FRMFiltro.fGetNumReg();
   if(frm.iCveSolicitante.value != "" || (frm.iEjercicioSol.value != "" && frm.iNumSolicitudBusca.value != "")){
...
function fReporteEjecutado(theWindow, aRes, aDato, cFiltro, cId, cError){
  ...
  if (cError == ""){
    ...
    frm.hdFiltro.value =  " RS.iCveSolicitante = " + frm.iCveSolicitante.value + " AND RS.lImpreso = " + ((frm.lImpresasBOX.checked)?"1":"0") + " ";
...
function fResultado(aRes,cId,cError,cNavStatus,iRowPag,cLlave){
  if(cError!=""){
    ...
    if(cId == "idMovimientos" && cError == ""){
      if(aRes.length == 0){
        fAbreSubWindow(true,"pg111020170",'yes',"yes","yes","yes",800,600,"","");
      }else{
      ...
      if(aRes.length > 0){
        if(aRes[0][0] == iEjercicioSel && aRes[0][1] == iNumSolSel){
          ...
          lSelAutomatico = true;
           ...
           k = 0;
           while(k < aRes.length && aRes[k][0] != iEjercicioSel && aRes[k][2] != iNumSel){
             k++;
           }
    ...
    if(cId == "DatosSolicBusquedaDirecta" && aRes && aRes.length && aRes.length > 0){
      if(FRMContenedor){
...
function fValidaMovimientos(){
  if(lImprocedente){
   ...
   select = "SELECT traregrefpago.IEJERCICIO, traregrefpago.INUMSOLICITUD, " +
      ...
      "FROM traregrefpago " +
      "WHERE traregrefpago.IEJERCICIO =  " + frm.iEjercicio.value +
...
function fBuscaRelacionadas(iEjer,iSol){
   ...
   select = "SELECT IEJERCICIO, INUMSOLICITUDPRINC, INUMSOLICITUDREL  " +
      ...
      "WHERE IEJERCICIO =  " + iEjer +
      " AND (INUMSOLICITUDPRINC = " + iSol +
      " OR INUMSOLICITUDREL = " + iSol + ")";
   frm.hdSelect.value = select;
   iEjercicioSel = iEjer;
   iNumSolSel = iSol;
...
function fBuscaRelacionadas(iEjer,iSol){
   ...
   select = "SELECT IEJERCICIO, INUMSOLICITUDPRINC, INUMSOLICITUDREL  " +
      "FROM TRASOLICITUDREL " +
      "WHERE IEJERCICIO =  " + iEjer +
      " AND (INUMSOLICITUDREL = " + frm.iNumSolicitud.value +
      " OR ";
   frm.hdSelect.value = select;
   fEngSubmite("pgConsulta.jsp","idMovimientos");
}*/
...
function fSelReg(aDato,iCol,lChecked){
  ...
  if(aDato[parseInt(aDato.length,10)-1] != "" && parseInt(aDato[parseInt(aDato.length,10)-1],10) == 1)
    lRequierePago = true;
  else
    lRequierePago = false;
...
function fReporte(){
  ...
  cNumerosRep = "1,";
  if(frm.lImpreso.value==1) cNumerosRep = "15,";
  cFiltrosRep = "";
  ...
  for (var i=0; i<aCBoxTra.length; i++){
    if (aCBoxTra[i] == true){
      ...
      if(aResSolicitudes[i][21] == 1 && aResSolicitudes[i][22] == 0 && aResSolicitudes[i][23] == 1 && aResSolicitudes[i][16] == "")
  ...
  if(cMsg != ""){
    lEjecuta = false;
    fAlert("\n\na)Las siguientes solicitudes no pueden entregarse por faltar registro de conceptos a pagar" + cMsg);
    return;
  }
  ...
  if (lEjecuta)
    ...
    if (frm.iEjercicio.value != "" && frm.iNumSolicitud.value != ""){
      if(frm.lImpresasBOX.checked)
        lContieneImpresas = true;
      if(parseInt(frm.lMovGenerados.value,10) == 1 && parseInt(frm.lPagoAnticipadoNoPagado.value,10) == 1 && lImprocedente == false && lRequierePago){
        cMsg = "\nEjercicio: " + frm.iEjercicio.value + " Número: " + frm.iNumSolicitud.value;
        if(parseInt(frm.lMovGenerados.value,10) == 1)
          fAlert("\n\nb)Las siguientes solicitudes no pueden entregarse por faltar registro de pagos para movimientos generados" + cMsg);
        else
          fAlert("\n\nb)Las siguientes solicitudes no pueden entregarse por no tener generados movimientos anticipados" + cMsg);
        return;
      }
      if(parseInt(frm.lPagoAnticipadoNoPagado.value,10) == 1 && lImprocedente == false && lRequierePago){
        cMsg = "\nEjercicio: " + frm.iEjercicio.value + " Número: " + frm.iNumSolicitud.value;
        fAlert("\n\nb)Las siguientes solicitudes no pueden entregarse por faltar registro de pagos anticipados requeridos." + cMsg);
        return;
      }
...
function fGetDatosSolicitanteBuscaDirecto(){
  if(frm.iEjercicio.value != "" && frm.iNumSolicitud.value != ""){
    frm.hdSelect.value = "SELECT SOL.iCvePersona, SOL.cRFC, SOl.cRPA, SOL.iTipoPersona, "+  //3
                         " SOL.cNomRazonSocial, SOL.cApPaterno, SOL.cApMaterno, SOL.cCorreoE, "+  //7
                         " SOL.cPseudonimoEmp, DOM.iCveDomicilio, DOM.iCveTipoDomicilio, "+  //10
                         " DOM.cCalle, DOM.cNumExterior, DOM.cNumInterior, DOM.cColonia, "+  //14
                         " DOM.cCodPostal, DOM.cTelefono, DOM.iCvePais, P.cNombre AS cDscPais, "+ //18
                         " DOM.iCveEntidadFed, EF.cNombre AS cDscEntidadFed, DOM.iCveMunicipio, "+ //21
                         " MUN.cNombre AS cDscMunicipio, DOM.iCveLocalidad, LOC.cNombre AS cDscLocalidad, "+ //24
                         " DOM.lPredeterminado, TD.cDscTipoDomicilio, RS.iCveTramite, RS.iCveModalidad, "+ //28
                         " RS.iEjercicio, RS.iNumSolicitud, RS.cNomAutorizaRecoger, RS.cObsTramite, "+ //32
                         " SOL.cNomRazonSocial || ' ' || SOL.cApPaterno || ' '|| SOL.cApMaterno AS cNomSolicitante, "+ //33
                         " DOM.cCalle || ' No. ' || DOM.cNumExterior || ' Int. ' || DOM.cNumInterior || ' COL. ' || DOM.cColonia || ', ' || MUN.cNombre || ' (' || EF.cAbreviatura ||'.)' AS cDomSolicitante, "+  //34
                         " RS.iCveRepLegal, RS.iCveDomicilioRepLegal, "+ //36
                         " REP.cNomRazonSocial || ' ' || REP.cApPaterno || ' '|| REP.cApMaterno AS cNomRepLegal, "+ //37
                         " DRL.cCalle || ' No. ' || DRL.cNumExterior || ' Int. ' || DRL.cNumInterior || ' COL. ' || DRL.cColonia || ', ' || MUNRL.cNombre || ' (' || EFRL.cAbreviatura ||'.)' AS cDomRepLegal, "+  //38
                         " RS.cDscBien, RS.iIdBien, RS.dUnidadesCalculo, TRA.CCVEINTERNA || ' - ' || TRA.CDSCBREVE AS cDscTramite "+ //42
...
function fPagaSol(){
  ...
  for (i=0;i<aCBox.length;i++){
    if (aCBox[i]==true){
      alert(aSolMarcadas.length+"  ++  "+i+"  ++  "+aRes[i][0]+"/"+aRes[i][1]);
      lSolMarcadas = true;
      aSolMarcadas[aSolMarcadas.length]=aRes[i][0]+"/"+aRes[i][1];
      //aSolMarcadas[aSolMarcadas.length][1]=aRes[i][1];
    }
  }
  if(lSolMarcadas==false)fAlert("Es necesario marcar al menos una solicitud.");
  else alert(aSolMarcadas);
}
