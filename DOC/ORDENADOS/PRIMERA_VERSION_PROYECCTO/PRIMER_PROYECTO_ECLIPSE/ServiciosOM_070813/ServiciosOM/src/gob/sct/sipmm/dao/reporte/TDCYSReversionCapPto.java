...
public class TDCYSReversionCapPto extends DAOBase{
  ...
  private int iDestinatario = Integer.parseInt(VParametros.getPropEspecifica("secretarioPublicoFed"),10);
  public TDCYSReversionCapPto(){
  }
    public Vector genReversionCapitania(String cQuery,String cNumFolio,String cCveOficinaOrigen,String cCveDeptoOrigen){
      ...
      if (vRegs.size() > 0){
        ...
        if(vDatos.getDate("dtPublicacionDOF")!=null)rep.comRemplaza("[cFechaTitulo]",Fechas.getDateSPN(vDatos.getDate("dtPublicacionDOF")));
        else rep.comRemplaza("[cFechaTitulo]"," ____de_________del_____ ");
        ...
        if(vNotifRec.size()>0){
          ...
          else rep.comRemplaza("[cFechaFolio]"," ____de_________del_____ ");
          if(vDatos.getDate("dtRecAsignacion")!=null)
            ...
            cFechaNotif = " ____de_________del_____ ";
          rep.comRemplaza("[cFechaNotif]",cFechaNotif);
        }
        else {
          rep.comRemplaza("[cFechaFolio]"," ____de_________del_____ ");
          rep.comRemplaza("[cFechaNotif]"," ____de_________del_____ ");
        }
    ...
    if(vcUbicacion.size()>=0){
      ...
      iPuerto = vUbicacion.getInt("iCvePuerto");
      rep.comRemplaza("[cPuerto]",puerto.toLowerCase());
    }
    else rep.comRemplaza("[cPuerto]","___________________");

    String qryPuertos =
        "SELECT P.ICVEOFICINAADSCRITA,O.CTITULAR,O.CCALLEYNO,O.CCOLONIA,O.CCODPOSTAL,O.CTELEFONO, O.CCORREOE, " +
        "EF.CNOMBRE AS CENTFED, M.CNOMBRE AS CMUNICIPIO, PA.CNOMBRE AS CPAIS FROM GRLPUERTO P " +
        "JOIN GRLOFICINA O ON P.ICVEOFICINAADSCRITA = O.ICVEOFICINA " +
        "join GRLPAIS pa on p.ICVEPAIS = pa.ICVEPAIS "+
        "join GRLENTIDADFED EF ON O.ICVEENTIDADFED = EF.ICVEENTIDADFED AND EF.ICVEPAIS = O.ICVEPAIS " +
        "JOIN GRLMUNICIPIO M ON P.ICVEPAIS = M.ICVEPAIS AND P.ICVEENTIDADFED = M.ICVEENTIDADFED AND P.ICVEMUNICIPIO = M.ICVEMUNICIPIO " +
        "WHERE P.ICVEPUERTO = "+iPuerto;
    Vector vPuertos = new Vector();

    try{
      vPuertos = super.FindByGeneric("", qryPuertos, dataSourceName);
    ...
    if (vPuertos.size() > 0){
      TVDinRep vDatosPuerto = (TVDinRep) vPuertos.get(0);
      rep.comRemplaza("[cNomCapitan]",vDatosPuerto.getString("CTITULAR"));
      rep.comRemplaza("[cCalleNumCap]",vDatosPuerto.getString("CCALLEYNO"));
      rep.comRemplaza("[cColoniaCap]",vDatosPuerto.getString("CCOLONIA"));
      rep.comRemplaza("[cCodigoPostalCap]",vDatosPuerto.getString("CCODPOSTAL"));
      rep.comRemplaza("[cMunicipioCap]",vDatosPuerto.getString("CMUNICIPIO"));
      rep.comRemplaza("[cEntidadCap]",vDatosPuerto.getString("CENTFED"));
      rep.comRemplaza("[cPaisCap]",vDatosPuerto.getString("CPAIS"));
