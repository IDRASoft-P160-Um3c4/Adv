 ...
 * @version 1.0
 */
public class TDGRLReporte extends DAOBase{
   ...
   * <p><b> insert into GRLReporte(iCveSistema,iCveModulo,iNumReporte,cNomReporte,lExcel,lWord,lPDF,lMultiPart,cPlantillaExcel,cPlantillaWord,cPlantillaPDF,cDAOEjecutar,cMetodoExcel,cMetodoWord,cMetodoPDF,iCveFormatoFiltro,lAutoImprimir,iNumCopias,lMostrarAplicacion,lParametroModificable) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) </b></p>
  ...
  public TVDinRep insert(TVDinRep vData,Connection cnNested) throws
    ...
    try{
      ...
      //AGREGAR AL ULTIMO ...
      Vector vcData = findByCustom("","select MAX(iCveSistema) AS iCveSistema from GRLReporte");
      if(vcData.size() > 0){
   ...
   * <p><b> update GRLReporte set cNomReporte=?, lExcel=?, lWord=?, lPDF=?, lMultiPart=?, cPlantillaExcel=?, cPlantillaWord=?, cPlantillaPDF=?, cDAOEjecutar=?, cMetodoExcel=?, cMetodoWord=?, cMetodoPDF=?, iCveFormatoFiltro=?, lAutoImprimir=?, iNumCopias=?, lMostrarAplicacion=?, lParametroModificable=? where iCveSistema = ? AND iCveModulo = ? AND iNumReporte = ?  </b></p>
   * <p><b> Campos Llave: iCveSistema,iCveModulo,iNumReporte, </b></p>
   * @param vData TVDinRep      - VO Dinámico que contiene a la entidad a Insertar.
   * @param cnNested Connection - Conexión anidada que permite que el método se encuentre dentro de una transacción mayor.
   * @throws DAOException       - Excepción de tipo DAO
