...
/**
 * <p>Title: TDCYSRequerimiento.java</p>
...
public class TDCYSRequerimiento extends DAOBase{
	...
	public TDCYSRequerimiento(){
	}

	public StringBuffer genRequerimiento(String cQuery,String cNumFolio,String cCveOficinaOrigen,String cCveDeptoOrigen){
		...
		TDCYSFolioProc dCYSFolioProc = new TDCYSFolioProc();
                System.out.println("*****    Hasta aqui ya creo instancia de TDCysFolioProc    *****");
                //TDActualizaRecargo dActualizaRecargo = new TDActualizaRecargo();
                System.out.println("*****    crea el constructor de Actualiza recargo");
		...
		.append("CPATitulo.iCveTipoTitulo ")
		...
		if (vRegs.size() > 0){
			...
			iCveTipoTitulo = vDatos.getInt("iCveTipoTitulo");
			...
			rep.comDespliegaAlineado("F",iReng,"K",iReng,cNumFolio,true,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng+=2;
			...
			rep.comDespliegaAlineado("F",iReng,"K",iReng,"Asunto: Requerimiento",true,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng+=2;
...
//		rep.comRemplaza("[cNombreRemitente]",dObten.dOficDepto.getTitular());
		...
		if(!vSustTit.isEmpty()){
			for(int i=0;i<vSustTit.size();i++){
				...
				System.out.println("Programa de Sustitucion de Titulos: " + vDinRep.getInt("lExiste"));
			}
		}
		...
		if(!vProMaeDesPor.isEmpty()){
			for(int i=0;i<vProMaeDesPor.size();i++){
				...
				System.out.println("Programa Maestro de Desarrollo Portuario: " + vDinRep.getInt("iEjercicio"));
				System.out.println("Programa Maestro de Desarrollo Portuario: " + vDinRep.getString("cMotivo"));
			}
		}
		...
		if(!vExtProMaeDesPor.isEmpty()){
			for(int i=0;i<vExtProMaeDesPor.size();i++){
				...
				System.out.println("Extracto del Programa Maestro de Desarrollo Portuario: " + vDinRep.getInt("lPresentacion"));
			}
		}
		...
		if(!vProOpeAnual.isEmpty()){
			for(int i=0;i<vProOpeAnual.size();i++){
				...
				System.out.println("Programa Operativo Anual: " + vDinRep.getInt("iEjercicio"));
				System.out.println("Programa Operativo Anual: " + vDinRep.getString("cMotivo"));
			}
		}
		...
		if(!vRegOpe.isEmpty()){
			for(int i=0;i<vRegOpe.size();i++){
				...
				System.out.println("Reglas de Operacion: " + vDinRep.getInt("iEjercicio"));
				System.out.println("Reglas de Operacion: " + vDinRep.getString("cMotivo"));
			}
		}
		...
		if(!vComOpe.isEmpty()){
			for(int i=0;i<vComOpe.size();i++){
				TVDinRep vDinRep = new TVDinRep();
				vDinRep = (TVDinRep) vRegOpe.get(i);
				System.out.println("Comite de Operacion: " + vDinRep.getString("cMotivo"));
			}
		}
		...
		if(!vOfiQue.isEmpty()){
			for(int i=0;i<vOfiQue.size();i++){
				TVDinRep vDinRep = new TVDinRep();
				vDinRep = (TVDinRep) vRegOpe.get(i);
				System.out.println("Oficina de Quejas: " + vDinRep.getString("cMotivo"));
			}
		}
		...
		if(!vSisCom.isEmpty()){
			for(int i=0;i<vSisCom.size();i++){
				TVDinRep vDinRep = new TVDinRep();
				vDinRep = (TVDinRep) vRegOpe.get(i);
				System.out.println("Sistema de Computo: " + vDinRep.getString("cMotivo"));
			}
		}
		...
		if(!vEdofin.isEmpty()){
			for(int i=0;i<vEdofin.size();i++){
				TVDinRep vDinRep = new TVDinRep();
				vDinRep = (TVDinRep) vRegOpe.get(i);
				System.out.println("Estados Financieros: " + vDinRep.getInt("iEjercicio"));
				System.out.println("Estados Financieros: " + vDinRep.getString("cMotivo"));
			}
		}
		...
		if(!vOtorgamiento.isEmpty()){
			for(int i=0;i<vOtorgamiento.size();i++){
				...
				if(cEjercicios.equalsIgnoreCase(""))
					cEjercicios = ""+vDinRep.getInt("iEjercicioNoPagado");
				else
					cEjercicios += ","+vDinRep.getInt("iEjercicioNoPagado");

				/*
				 * Derecho de Otorgamiento.
				 * Artículo 167 LFD (Antes del 2006)
				 */

				String[] aEjercicios = cEjercicios.split(",");
				Vector vcImportes = new Vector();
				TVDinRep dinImportes = new TVDinRep();
                                TVDinRep vcOtorgamiento = new TVDinRep();
		                 //TDCYSOtorgamiento dCYSOtorgamiento = new TDCYSOtorgamiento();
                                try{
                                  vOtorgamiento = dCYSOtorgamiento.findBySitObligacion("",
                                      iCveTitulo,dtConcesion);
                                }catch(Exception ex){
                                  ex.printStackTrace();
                                }
				...
				for(int j = 0; j < aEjercicios.length; j++){
					StringBuffer sb = new StringBuffer();
					sb.append("SELECT ");
					sb.append("REF.DIMPORTESINAJUSTE AS DIMPORTESINAJUSTE, ");
					sb.append("REF.DIMPORTEAJUSTADO AS DIMPORTEAJUSTADO ");
					sb.append("FROM CPATIPOTITULO TIT ");
					sb.append("JOIN TRACONCEPTOPAGO CON ON TIT.ICVECONCEPTO = CON.ICVECONCEPTO ");
					sb.append("JOIN TRAREFERENCIACONCEPTO REF ON CON.ICVECONCEPTO = REF.ICVECONCEPTO ");
					sb.append("WHERE REF.IEJERCICIO = " + aEjercicios[j]);
					sb.append(" AND TIT.ICVETIPOTITULO = " + iCveTipoTitulo);

					try {
						vcImportes = super.FindByGeneric("",sb.toString(), dataSourceName);
					} catch (SQLException e) {
						e.printStackTrace();
					}

					if(vcImportes.size() > 0)
						dinImportes = (TVDinRep) vcImportes.get(0);

				}

				rep.comDespliegaAlineado("C", iReng,"I",iReng,"- Derecho de Otorgamiento del título. Art. 167 de la Ley Federal de Derechos" +
						" (antes del 2006).",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
				iReng++;
				rep.comDespliegaAlineado("C", iReng,"I",iReng,"Ejercicio(s): " + cEjercicios + ", con actualizaciones (3) y recargos (4)" +
						" por pago extemporáneo." ,false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
				iReng+=2;
				rep.comDespliegaCombinado("Base", "C", iReng, "D", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Periodo de Pago", "E", iReng, "H", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Fecha de Pago (1)", "I", iReng, "I", iReng+2, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Fecha de Presentación (2)", "J", iReng, "J", iReng+2, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Descripción", "K", iReng, "K", iReng+2, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Importe Pagado", "L", iReng, "L", iReng+2, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Importe Correcto", "M", iReng, "M", iReng+2, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Diferencia", "N", iReng, "N", iReng+2, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				iReng++;
				rep.comDespliegaCombinado("Ejercicio", "C", iReng, "C", iReng+1, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Cuota", "D", iReng, "D", iReng+1, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Inicial", "E", iReng, "F", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				rep.comDespliegaCombinado("Final", "G", iReng, "H", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
				iReng++;
				rep.comDespliegaAlineado("E", iReng, "Mes", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
				rep.comDespliegaAlineado("F", iReng, "Año", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
				rep.comDespliegaAlineado("G", iReng, "Mes", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
				rep.comDespliegaAlineado("H", iReng, "Año", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
				rep.comBordeRededor("E", iReng, "E", iReng, 1, 0);
				rep.comBordeRededor("F", iReng, "F", iReng, 1, 0);
				rep.comBordeRededor("G", iReng, "G", iReng, 1, 0);
				rep.comBordeRededor("H", iReng, "H", iReng, 1, 0);
				iReng++;
                                String cOtorgamiento =
                                    "SELECT " +
                                    "CCP.IEJERCICIO,RC.DIMPORTEAJUSTADO, " +
                                    "CO.IPERIODOINIPAGO,CO.IEJERCICIOINIPAGO,CO.IPERIODOFINPAGO,CO.IEJERCICIOFINPAGO, " +
                                    "CO.DTREGISTRO,CO.DTPRESENTACION AS DTPRESENTACIONPAGO, " +
                                    "CCP.IPERIODO,CCP.DTPRESENTACION  as dtLimPresentacion, " +
                                    "CO.DIMPDERECHOS,CO.DIMPACTUALIZACION,CO.DIMPRECARGOS,CO.DIMPTOTALPAGADO " +
                                    "FROM CYSCONFIGURAPRESENTACION CCP " +
                                    "LEFT JOIN CPATITULO CT ON CCP.ICVETITULO = CT.ICVETITULO " +
                                    "LEFT JOIN CPATIPOTITULO TT ON CT.ICVETIPOTITULO = TT.ICVETIPOTITULO " +
                                    "LEFT JOIN TRAREFERENCIACONCEPTO RC ON TT.ICVECONCEPTO = RC.ICVECONCEPTO " +
                                    "LEFT JOIN CYSOTORGAMIENTO CO ON CCP.ICVETITULO = CO.ICVETITULO " +
                                    "   AND CCP.IEJERCICIO = CO.IEJERCICIO " +
                                    "   AND CCP.IPERIODO = CO.IPERIODOINIPAGO " +
                                    "WHERE CCP.ICVETITULO = " + iCveTitulo +
                                    "   AND CCP.ICVEOBLIGACION = 11 " +
                                    "   AND CCP.IEJERCICIO = "+iEjercicio;
                                Vector vcOtorg = new Vector();
                                try{
                                  vcOtorg = super.FindByGeneric("",cOtorgamiento,dataSourceName);
                                } catch (SQLException e) {
                                   e.printStackTrace();
                                }

                                double dTotalDerechos = 0.0;
                                double dTotalActualizacion = 0.0;
                                double dTotalRecargos = 0.0;
                                double dTotalPagado = 0.0;
                                int iCount2 = 0;
                                double dCuota = 0.0;
                                //TDActualizaRecargo ARec = new TDActualizaRecargo();
                                java.sql.Date dtPago=null, dtAdeudo = null;


                                for(int iCount=0;iCount<vcOtorg.size();iCount++){
                                  TVDinRep vOtorg = (TVDinRep) vcOtorg.get(iCount);
                                  if(vOtorg.getDouble("DIMPORTEAJUSTADO")>0)
                                    dCuota = vOtorg.getDouble("DIMPORTEAJUSTADO");
                                  if(!vOtorg.getString("DTPRESENTACIONPAGO").equals("null"))
                                    dtPago = vOtorg.getDate("DTPRESENTACIONPAGO");
                                  if(!vOtorg.getString("dtLimPresentacion").equals("null"))
                                    dtAdeudo = vOtorg.getDate("dtLimPresentacion");
                                  rep.comDespliegaAlineado("C",iReng+iCount,(vOtorg.getInt("IEJERCICIO")>0)?
                                                                     (""+vOtorg.getInt("iEjercicio")):"",
                                                                     false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
                                  rep.comBordeRededor("C",iReng+iCount,"C",iReng+iCount,1,0);
                                  rep.comDespliegaAlineado("D",iReng+iCount,(vOtorg.getDouble("DIMPORTEAJUSTADO")>0)?
                                                                     (""+vOtorg.getDouble("DIMPORTEAJUSTADO")):"",
                                                                     false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
                                  rep.comBordeRededor("D",iReng+iCount,"D",iReng+iCount,1,0);
                                  rep.comDespliegaAlineado("E",iReng+iCount,(vOtorg.getInt("iPeriodoIniPago")>0)?
                                                                     (""+vOtorg.getInt("iPeriodoIniPago")):"",
                                                                     false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
                                  rep.comBordeRededor("E",iReng+iCount,"E",iReng+iCount,1,0);
                                  rep.comDespliegaAlineado("F",iReng+iCount,(vOtorg.getInt("iEjercicioIniPago")>0)?
                                                                     (""+vOtorg.getInt("iEjercicioIniPago")):"",
                                                                     false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
                                  rep.comBordeRededor("F",iReng+iCount,"F",iReng+iCount,1,0);
                                  rep.comDespliegaAlineado("G",iReng+iCount,(vOtorg.getInt("iPeriodoFinPago")>0)?
                                                                     (""+vOtorg.getInt("iPeriodoFinPago")):"",
                                                                     false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
                                  rep.comBordeRededor("G",iReng+iCount,"G",iReng+iCount,1,0);
                                  rep.comDespliegaAlineado("H",iReng+iCount,(vOtorg.getInt("iEjercicioFinPago")>0)?
                                                                     (""+vOtorg.getInt("iEjercicioFinPago")):"",
                                                                     false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
                                  rep.comBordeRededor("H",iReng+iCount,"H",iReng+iCount,1,0);
                                  rep.comDespliegaAlineado("I",iReng+iCount,(vOtorg.getDate("DTREGISTRO")!=null)?
                                                                     (""+vOtorg.getDate("DTREGISTRO")):"",
                                                                     false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
                                  rep.comBordeRededor("I",iReng+iCount,"I",iReng+iCount,1,0);
                                  rep.comDespliegaAlineado("J",iReng+iCount,(vOtorg.getDate("DTPRESENTACIONPAGO")!=null)?
                                                                     (""+vOtorg.getDate("DTPRESENTACIONPAGO")):"",
                                                                     false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
                                  rep.comBordeRededor("j",iReng+iCount,"J",iReng+iCount,1,0);
                                  dTotalDerechos += vOtorg.getDouble("DIMPDERECHOS");
                                  dTotalActualizacion += vOtorg.getDouble("DIMPACTUALIZACION");
                                  dTotalRecargos += vOtorg.getDouble("DIMPRECARGOS");
                                  dTotalPagado += vOtorg.getDouble("DIMPTOTALPAGADO");
                                  iCount2 = iCount - 3;
                                }
                                //TVDinRep wsRecargo = ARec.findActRecargo(dtAdeudo,dtPago,dCuota);

                                rep.comDespliegaAlineado("K", iReng, "Derechos", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
                                rep.comDespliegaAlineado("L", iReng, ""+dTotalDerechos, false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
                                //rep.comDespliegaAlineado("M",iReng,""+wsRecargo.getFloat("ImporteActualizado"),false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
				rep.comBordeRededor("C", iReng, "C", iReng, 1, 0);
				rep.comBordeRededor("D", iReng, "D", iReng, 1, 0);
				rep.comBordeRededor("E", iReng, "E", iReng, 1, 0);
				rep.comBordeRededor("F", iReng, "F", iReng, 1, 0);
				rep.comBordeRededor("G", iReng, "G", iReng, 1, 0);
				rep.comBordeRededor("H", iReng, "H", iReng, 1, 0);
				rep.comBordeRededor("I", iReng, "I", iReng, 1, 0);
				rep.comBordeRededor("J", iReng, "J", iReng, 1, 0);
				rep.comBordeRededor("K", iReng, "K", iReng, 1, 0);
				rep.comBordeRededor("L", iReng, "L", iReng, 1, 0);
				rep.comBordeRededor("M", iReng, "M", iReng, 1, 0);
				rep.comBordeRededor("N", iReng, "N", iReng, 1, 0);
				iReng++;
                                rep.comDespliegaAlineado("K", iReng, "Actualización (3)", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
                                rep.comDespliegaAlineado("L", iReng, ""+dTotalActualizacion, false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
                                //rep.comDespliegaAlineado("M",iReng,""+wsRecargo.getFloat("DifActualizacion"),false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
				rep.comBordeRededor("C", iReng, "C", iReng, 1, 0);
				rep.comBordeRededor("D", iReng, "D", iReng, 1, 0);
				rep.comBordeRededor("E", iReng, "E", iReng, 1, 0);
				rep.comBordeRededor("F", iReng, "F", iReng, 1, 0);
				...
				rep.comBordeRededor("N", iReng, "N", iReng, 1, 0);
				iReng++;
                                rep.comDespliegaAlineado("K", iReng, "Recargos (4)", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
                                rep.comDespliegaAlineado("L", iReng, ""+dTotalRecargos, false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
                                //rep.comDespliegaAlineado("M",iReng,""+wsRecargo.getFloat("ImporteRecargo"),false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
				rep.comBordeRededor("C", iReng, "C", iReng, 1, 0);
				rep.comBordeRededor("D", iReng, "D", iReng, 1, 0);
				rep.comBordeRededor("E", iReng, "E", iReng, 1, 0);
				rep.comBordeRededor("F", iReng, "F", iReng, 1, 0);
				...
				rep.comBordeRededor("N", iReng, "N", iReng, 1, 0);
				iReng++;
                                rep.comDespliegaAlineado("K", iReng, "Total", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
                                rep.comDespliegaAlineado("L", iReng, ""+dTotalPagado, false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
                                //rep.comDespliegaAlineado("M",iReng,""+wsRecargo.getFloat("ImporteTotal"),false,rep.getAT_HCENTRO(),rep.getAT_VCENTRO());
				rep.comBordeRededor("C", iReng, "C", iReng, 1, 0);
				rep.comBordeRededor("D", iReng, "D", iReng, 1, 0);
				rep.comBordeRededor("E", iReng, "E", iReng, 1, 0);
				rep.comBordeRededor("F", iReng, "F", iReng, 1, 0);
				...
				iReng++;
                                iReng += iCount2;
				rep.comDespliegaAlineado("C", iReng,"I",iReng,"1. La fecha de pago debe ser antes del día 31 de Enero del ejercicio en " +
						"cuestión, de acuerdo al",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
				iReng++;
				rep.comDespliegaAlineado("C", iReng,"I",iReng,"art. 4 párrafo quinto de la Ley Federal de Derechos.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
				iReng++;
				rep.comDespliegaAlineado("C", iReng,"I",iReng,"2. La presentación del comprobante de pago ante esta Dirección General"+
						" debe ser antes del 15 de ",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
				iReng++;
				rep.comDespliegaAlineado("C", iReng,"I",iReng,"febrero del ejercicio, de acuerdo al art. 4 párrafo quinto, de la Ley Federal de Derechos",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
				iReng++;
				rep.comDespliegaAlineado("C", iReng,"I",iReng,"3. Las actualizaciones son de acuerdo al Art. 17 - A del Código Fiscal de la " +
						"Federación.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
				iReng++;
				rep.comDespliegaAlineado("C", iReng,"I",iReng,"4. Los recargos son de acuerdo al Art. 21 del Código Fiscal de la Federación",
						false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
				iReng+=2;

				System.out.println("Otorgamiento \n Ejercicio No Pagado: " + vDinRep.getInt("iEjercicioNoPagado"));
				System.out.println("\n Periodo no pagado: " + vDinRep.getString("cPeriodoNoPagado"));
			}
		...
		if(!vDerSenalamiento.isEmpty()){
			for(int i=0;i<vDerSenalamiento.size();i++){
				...
				System.out.println("Derecho del Señalamiento Marítimo: " + vDinRep.getInt("iExiste"));
			}

			rep.comDespliegaAlineado("C", iReng,"I",iReng,"- Derecho del Señalamiento Marítimo. Art. 169 - A de la Ley Federal de Derechos.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng+=2;

		}

		/**
		 * Derecho de uso, goce y aprovechamiento 232 - I LFD
		 */

		StringBuffer sbAprov = new StringBuffer();
		sbAprov.append("SELECT ")
		.append("CYSDERECHOAPROVECHAMIENTO.IEJERCICIO, ")
		.append("CYSDERECHOAPROVECHAMIENTO.DTREGISTRO, ")
		.append("CYSDERECHOAPROVECHAMIENTO.DTPRESENTACION, ")
		.append("CYSDERECHOAPROVECHAMIENTO.DTPAGO, ")
		.append("CYSDERECHOAPROVECHAMIENTO.IEJERCICIOINIPAGO, ")
		.append("CYSDERECHOAPROVECHAMIENTO.IPERIODOINIPAGO, ")
		.append("CYSDERECHOAPROVECHAMIENTO.IEJERCICIOFINPAGO, ")
		.append("CYSDERECHOAPROVECHAMIENTO.IPERIODOFINPAGO, ")
		.append("CYSDERECHOAPROVECHAMIENTO.DIMPDERECHOS, ")
		.append("CYSDERECHOAPROVECHAMIENTO.DIMPACTUALIZACION, ")
		.append("CYSDERECHOAPROVECHAMIENTO.DIMPRECARGOS, ")
		.append("CYSDERECHOAPROVECHAMIENTO.DIMPTOTALPAGADO ")
		.append("FROM CYSDERECHOAPROVECHAMIENTO ")
		.append("WHERE CYSDERECHOAPROVECHAMIENTO.ICVETITULO = " + iCveTitulo);

		Vector vAprov = new Vector();
		TVDinRep dinAprov = new TVDinRep();

		try {
			vAprov = super.FindByGeneric("", sbAprov.toString(), dataSourceName);
		} catch (SQLException e) {
			e.printStackTrace();
		}

		if(vAprov.isEmpty()){
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"- Derechos de uso, goce o aprovechamiento de inmuebles Art. 232, fracción I, de " +
					"la Ley Federal de ",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"Derechos",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"Ejercicio(s) ____, con actualizaciones (3) y recargos (4) por pago extemporáneo.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng+=2;
			...
			rep.comDespliegaCombinado("Diferencia", "N", iReng, "N", iReng+2, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
			iReng++;
			rep.comDespliegaCombinado("Porcentaje 7.5%","C", iReng, "D", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
			rep.comDespliegaCombinado("Inicial","E", iReng, "F", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
			rep.comDespliegaCombinado("Final","G", iReng, "H", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
			iReng++;
			rep.comDespliegaAlineado("C", iReng, "Avalúo", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("D", iReng, "Fecha", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("E", iReng, "Mes", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("F", iReng, "Año", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("G", iReng, "Mes", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("H", iReng, "Año", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			...
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"1. La fecha de pago debe ser antes del día 17 del mes siguiente de causación de" +
					" manera bimestral,",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"de acuerdo al art. 234 de la Ley Federal de Derechos.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"2. La presentación del comprobante de pago ante esta Dirección General, debe" +
					" realizarse dentro de",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"los 5 días siguientes al de su realización.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"3. Las actualizaciones son de acuerdo al Art. 17 - A del Código Fiscal de la Federación.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"4. Los recargos son de acuerdo al Art. 21 del Código Fiscal de la Federación.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng+=2;

		}else{
			// Validar en caso de que haya pagado, que haya pagado en tiempo y los importes correctos.
		}
		...
		System.out.println(" 232 A ---------> \n\n\n " + vDerExplotacion.size());
		...
		if(vDerExplotacion.isEmpty()){
			...
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"- Derechos de uso, goce o explotación de bienes de dominio público, que " +
					"queden afectos a ",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"servicios públicos o para la prestación de Servicios Portuarios de acuerdo " +
					"al Art. 232 - A",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"de la Ley Federal de Derechos.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng+=2;
			...
			rep.comDespliegaCombinado("Diferencia", "N", iReng, "N", iReng+2, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
			iReng++;
			rep.comDespliegaCombinado("Porcentaje 5%","C", iReng, "D", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
			rep.comDespliegaCombinado("Inicial","E", iReng, "F", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
			rep.comDespliegaCombinado("Final","G", iReng, "H", iReng, rep.getAT_COMBINA_CENTRO(), rep.getAT_VCENTRO(), false, 0, true, false, 1, 0);
			iReng++;
			rep.comDespliegaAlineado("C", iReng, "Ingresos", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("D", iReng, "Importe", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("E", iReng, "Mes", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("F", iReng, "Año", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("G", iReng, "Mes", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			rep.comDespliegaAlineado("H", iReng, "Año", false, rep.getAT_HCENTRO(), rep.getAT_VCENTRO());
			...
			//Validar si pago, que haya pagado en tiempo y el importe correcto
		}

		/**
		 * Contraprestación Art. 37 de la Ley De Puertos
		 */

		StringBuffer sbContraPrest = new StringBuffer();
		sbContraPrest.append("SELECT ")
		.append("IEJERCICIO, ")
		.append("DTREGISTRO, ")
		.append("DTPAGO, ")
		.append("IEJERCICIOINIPAGO, ")
		.append("IPERIODOINIPAGO, ")
		.append("IEJERCICIOFINPAGO, ")
		.append("IPERIODOFINPAGO, ")
		.append("DIMPDERECHOS, ")
		.append("DIMPACTUALIZACION, ")
		.append("DIMPRECARGOS, ")
		.append("DIMPTOTALPAGADO, ")
		.append("CCVEDGPOPRECARGOS ")
		.append("FROM CYSAPROVECHAMIENTO ")
		.append("WHERE ICVETITULO = " + iCveTitulo);

		Vector vContraP = new Vector();
		TVDinRep dinContrap = new TVDinRep();

		try {
			vContraP = super.FindByGeneric("", sbContraPrest.toString(), dataSourceName);
		} catch (SQLException e) {
			e.printStackTrace();
		}

		if(vContraP.isEmpty()){
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"- Contraprestación para concesiones, art. 37 de la Ley de Puertos, por el uso, " +
					"aprovechamiento ",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"y explotación de los bienes de dominio público y de los servicios concesionados",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng++;
			rep.comDespliegaAlineado("C", iReng,"I",iReng,"de la Ley Federal de Derechos.",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng+=2;
			...
			//Validar las fechas de pago y los montos.
		}
		...
		iReng+=3;
		rep.comDespliegaAlineado("B", iReng,"I",iReng,"AECHR/ARS",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
		iReng++;
		rep.comDespliegaAlineado("B", iReng,"I",iReng,"REQUERIMIENTO " + cNombrePersonaDest,false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
		iReng++;
		rep.comDespliegaAlineado("B", iReng,"I",iReng,"No. de Expediente",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
		float fTam = 8.0f;
	...
	public Vector genRequerimientoWord(String cQuery,String cNumFolio,String cCveOficinaOrigen,String cCveDeptoOrigen){
		...
		String cNombreAPI="",cNomCesionario="",cNumTitulo="",cAsunto="Requerimiento";
		...
		if (vRegs.size() > 0){
			...
			rep.comRemplaza("[dtTitulo]", vDatos.getDate("dtIniVigenciaTitulo") != null ? Fechas.getFechaDDMMYYYY(vDatos.getDate("dtIniVigenciaTitulo"),"/"):"");
			rep.comRemplaza("[dtVigencia]", vDatos.getDate("dtVigenciaTitulo") != null ? Fechas.getFechaDDMMYYYY(vDatos.getDate("dtVigenciaTitulo"),"/"):"");
			rep.comRemplaza("[dtInicio]",vDatos.getDate("dtIniVigenciaTitulo")!=null?Fechas.getFechaDDMMYYYY(vDatos.getDate("dtIniVigenciaTitulo"),"/"):"");
		...
		if(!vOtorgamiento.isEmpty()){
			for(int i=0;i<vOtorgamiento.size();i++){
				TVDinRep vDinRep = new TVDinRep();
				vDinRep = (TVDinRep) vOtorgamiento.get(i);
				System.out.println("Otorgamiento: " + vDinRep.getInt("iEjercicioNoPagado"));
				System.out.println("Otorgamiento: " + vDinRep.getString("cPeriodoNoPagado"));
			}
		}
		...
		if(!vDerExplotacion.isEmpty()){
			for(int i=0;i<vDerExplotacion.size();i++){
				TVDinRep vDinRep = new TVDinRep();
				vDinRep = (TVDinRep) vDerExplotacion.get(i);
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getDouble("dIngresos"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getDouble("dImporte"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getInt("iMesI"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getInt("iEjercicioI"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getInt("iMesF"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getInt("iEjercicioF"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getDate("dtPago"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getDate("dtPresentacion"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getString("cClave"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getDouble("dImpPagado"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getDouble("dImpCorrecto"));
				System.out.println("Derecho y Explotacion 232 - A LFD: " + vDinRep.getDouble("dDiferencia"));
	...
	public Vector genNotificacion(String cQuery,String cNumFolio,String cCveOficinaOrigen,String cCveDeptoOrigen){
		...
		cNombreAPI = aFiltros[5];
		int iCveOficDest = Integer.parseInt(aFiltros[6]);
		int iTipoOficio = Integer.parseInt(aFiltros[7]);
		...
		try{
			...
			TVDinRep vDinRep4 = new TVDinRep();
			vDinRep3.put("iEjercicio",          vDinRep.getInt("iEjercicio"));
			vDinRep3.put("iMovProcedimiento",   iMovProcedimiento);
			vDinRep3.put("iMovFolioProc",       iMovFolioProc);
			...
			vDinRep4 = dCYSNotOficio.updNotificacion(vDinRep3,null);
	...
	public Vector genRecordatorio(String cQuery,String cNumFolio,String cCveOficinaOrigen,String cCveDeptoOrigen){
		...
		try{
			...
			vDinRep3.put("iMovFolioProc",       iMovFolioProc);
			vDinRep3.put("iMovCitaNotificacion",iMovCitaNotificacion);
			vDinRep3.put("iEjercicioRec",       vDinRep2.getInt("iEjercicio"));
			vDinRep3.put("iMovProcedimientoRec",vDinRep2.getInt("iMovProcedimiento"));
			vDinRep3.put("iMovFolioProcRec",    vDinRep2.getInt("iMovFolioProc"));

			vDinRep3 = dCYSNotOficio.updRecordatorio(vDinRep3,null);
