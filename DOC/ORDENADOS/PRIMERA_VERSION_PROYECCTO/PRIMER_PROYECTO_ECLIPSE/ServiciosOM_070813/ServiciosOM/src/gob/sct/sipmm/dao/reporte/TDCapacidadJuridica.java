...
public class TDCapacidadJuridica extends DAOBase{
  ...
  public StringBuffer WordOficio(String cQuery){
	  System.out.println("************** WORD OFICIO *******************");	  
	  ...
	  System.out.println("::::::Numero de Ficheros:::::" + i);
	  ...
	  if (ficheros == null){		  
	   ...
	   for (int x=0;x<ficheros.length;x++){
	     System.out.println(":::::fich::::::" + ficheros[x]);
	...
	if(vRegs1.size()>0){
	  ...
	  for (int countx = 0; countx < cParametros.length; countx++) {		  
			...
			System.out.println(">>>>>>>>>>::::::::::Parametro:" + countx + ":: valor :: " + valor);
	  }
	
	  rep.iniciaReporte();
	  System.out.println("*****  Tamaño del vector  "+vRegs.size());
	  ...
	  if (vRegs.size() > 0){
		...
		try {
			System.out.println(":::: Parametros Antes Reporte ::::" + "platilla " + rutaPlantilla + plantilla + "Archivo Final " + rutaArchivoFinal + archivoFinal);
			System.out.println("::Resultado de llenado Plantilla Agente Naviero::" + integracion.generaDocSustText(plantilla, rutaPlantilla, archivoFinal, rutaArchivoFinal, param));			
		} catch (Exception e) {		
			e.printStackTrace();
		}		
