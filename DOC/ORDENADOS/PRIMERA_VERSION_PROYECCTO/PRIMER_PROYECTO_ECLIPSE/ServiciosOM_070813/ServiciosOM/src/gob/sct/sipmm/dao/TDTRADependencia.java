 ...
 * @version 1.0
 */
public class TDTRADependencia extends DAOBase{
  ...
  public TVDinRep insert(TVDinRep vData,Connection cnNested) throws
    ...
    try{
      ...
      String lSQL =
          "insert into TRADependencia(iCveTramite,iCveModalidad,iCveTramiteHijo,iCveModalidadHijo) values (?,?,?,?)";
      ...
      String lReqNuevo = "SELECT ICVETRAMITE,ICVEMODALIDAD,ICVEREQUISITO,IORDEN,LREQUERIDO " +
                           "FROM TRAREQXMODTRAMITE rmt " +
                           "where rmt.ICVETRAMITE="+vData.getInt("iCveTramiteHijo")+
                           " and rmt.ICVEMODALIDAD="+vData.getInt("iCveModalidadHijo")+
                           " and ICVEREQUISITO not in ( " +
                           "    SELECT ICVEREQUISITO FROM TRAREQXMODTRAMITE "+
                           "    where ICVETRAMITE="+vData.getInt("iCveTramite")+
                           "     and icvemodalidad= " +vData.getInt("iCveModalidad")+
                           ") ";
     Vector vcReqNuevo = this.findByNessted("",lReqNuevo,conn);
     if(vcReqNuevo.size()>0){
       for(int i=0;i<vcReqNuevo.size();i++){
         TVDinRep vReqNuevo = (TVDinRep) vcReqNuevo.get(i);
         String cSql = "Insert Into TRAREQXMODTRAMITE (iCveTramite,iCveModalidad,iCveRequisito,iOrden,lRequerido) values (?,?,?,?,?)";
         lPStmt1 = conn.prepareStatement(cSql);
         lPStmt1.setInt(1,vData.getInt("iCveTramite"));
         lPStmt1.setInt(2,vData.getInt("iCveModalidad"));
         lPStmt1.setInt(3,vReqNuevo.getInt("ICVEREQUISITO"));
         lPStmt1.setInt(4,vReqNuevo.getInt("iOrden"));
         lPStmt1.setInt(5,vReqNuevo.getInt("lRequerido"));
  ...
  public boolean delete(TVDinRep vData,Connection cnNested)throws
    ...
    try{
      ...
      String lSQL = "delete from TRADependencia where iCveTramite = ? AND iCveModalidad = ? AND iCveTramiteHijo = ? AND iCveModalidadHijo = ?  ";
      //...
