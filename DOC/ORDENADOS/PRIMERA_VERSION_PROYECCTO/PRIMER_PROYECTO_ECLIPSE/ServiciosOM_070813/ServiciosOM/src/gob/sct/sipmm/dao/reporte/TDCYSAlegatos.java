...
public class TDCYSAlegatos extends DAOBase{
	...
	public TDCYSAlegatos(){
	}

	public StringBuffer genAlegatos(String cQuery,String cNumFolio,String cCveOficinaOrigen,String cCveDeptoOrigen){
		...
		int iCveTitular = 0, iCveTipoTitulo = 0;
		int iCveDomicilio = 0;
		int iCveRepLegal = 0;
		int iReng = 1;
		...
		.append("GRLPersona.iCvePersona as iCveTitular, ")
		...
		if (vRegs.size() > 0){
			...
			iCveTitular = vDatos.getInt("iCveTitular");
			...
			rep.comDespliegaAlineado("F",iReng,"K",iReng,"Asunto: Se concede plazo para producir Alegatos.",true,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
			iReng+=2;
...
//		 Información de la Sanción Administrativa
		Vector vDatosSegProced = new Vector();
		TVDinRep tvData;
		TDCYSSegProcedimiento dSegProced = new TDCYSSegProcedimiento();

		try {
			vDatosSegProced = dSegProced.findByOficioNotRec(iEjercicio,iMovProcedimiento,0);
		} catch (DAOException e) {
			e.printStackTrace();
		}

		String cOficioSan = "", cFechaSan = "";

		if(vDatosSegProced.size() > 0){
			tvData = (TVDinRep) vDatosSegProced.get(0);
			cOficioSan = tvData.getString("cDigitosFolio") + "." +
			tvData.getInt("iConsecutivo") + "/" + tvData.getInt("iEjercicioFol");
			cFechaSan = tvData.getDate("dtAsignacion") != null?Fecha.getFechaDDMMMMMYYYY(tvData.getDate("dtAsignacion")," de "):"";
		}

		String cPrimerParrafo = "Me refiero al procedimiento administrativo de Sanción instaurado en su contra mediante oficio " +
				cOficioSan + " de " + cFechaSan +", por los incumplimientos a diversas obligaciones establecidas en el título " +
				"referido, que le otorgó " + cOtorgado ;

		iReng++;
		rep.comDespliegaAlineado("B", iReng,"K",iReng+2,cPrimerParrafo,false,rep.getAT_COMBINA_IZQUIERDA(),rep.getAT_VSUPERIOR());
		rep.comAlineaRango("B",iReng,"K",iReng + 2,rep.getAT_HJUSTIFICA());
		iReng+=4;
		String cSegundoParrafo = "Sobre el particular, le comunico que de la revisión de las constancias que obran en el expediente " +
				"administrativo en que se actúa, se desprende que persiste el incumplimiento a las siguientes condiciones, " +
				"habiendo concluido la tramitación del procedimiento administrativo instaurado y previo a su resolución, con " +
				"fundamento en el artículo 56 de la Ley Federal de Procedimiento Administrativo, se le concede un plazo de 10 días " +
				"hábiles contados a partir de la notificación del presente, para que produzca sus alegatos en el referido procedimiento " +
				"y se le requiere para que dentro de dicho término acredite su cumplimiento.";

		rep.comDespliegaAlineado("B",iReng,"K",iReng + 3,cSegundoParrafo,false,rep.getAT_COMBINA_IZQUIERDA(),rep.getAT_VSUPERIOR());
...
//		Textos final doc.
		...
		String cPrimerParrafoFin = "Lo anterior, con fundamento en los artículos 14 y 16 de la Constitución Política de " +
				"los Estados Unidos Mexicanos; 36 fracciones XIX y XX de la Ley Orgánica de la Administración Pública Federal; " +
				"16 fracción IV, 20, 63, 64 de la Ley de Puertos; 16 de la Ley General de Bienes Nacionales; 3, 13, 32, 35 " +
				"fracción II y 56 de la Ley Federal de Procedimiento Administrativo; 27 fracciones IV, XI, XX, XXI y XXIII del " +
		...
		iReng++;
		rep.comDespliegaAlineado("B", iReng,"I",iReng,"REQUERIMIENTO (Nombre o Razón)",false,rep.getAT_HIZQUIERDA(),rep.getAT_VSUPERIOR());
		iReng++;
...
//		rep.comRemplaza("[cNombreRemitente]",dObten.dOficDepto.getTitular());
	...
	public Vector genAlegatosWord(String cQuery,String cNumFolio,String cCveOficinaOrigen,String cCveDeptoOrigen){
		...
		int iCveTitular = 0;
		int iCveDomicilio = 0;
		int iCveRepLegal = 0, iCveTipoTitulo = 0;
		java.sql.Date dtConcesion = null;
		java.sql.Date dtHoy = Fechas.TodaySQL();
		...
		sbAsunto.append("Se concede plazo para producir Alegatos.");

		this.setDatosFolio(cNumFolio);

		System.out.print("Valor del cNumFolio: " + cNumFolio);
		...
		"GRLPersona.iCvePersona as iCveTitular, " +
		...
		if (vRegs.size() > 0){
			...
			if(vDatosSegProced.size() > 0){
				...
				rep.comRemplaza("[cNumOficioSan]", tvData.getString("cDigitosFolio") + "." +
						tvData.getInt("iConsecutivo") + "/" + tvData.getInt("iEjercicioFol"));

				rep.comRemplaza("[cFechaSancion]", (tvData.getDate("dtAsignacion") != null)?Fecha.getFechaDDMMMMMYYYY(tvData.getDate("dtAsignacion")," de "):"");
				if(iCveTipoTitulo== 1)
					rep.comRemplaza("[cOtorgoSan]","El Presidente de la República a través de la Dirección General de Puertos");
				else if((iCveTipoTitulo== 2) || (iCveTipoTitulo== 3))
					rep.comRemplaza("[cOtorgoSan]","La Dirección General de Puertos");

			}else{
				rep.comRemplaza("[cNumOficioSan]","");
				rep.comRemplaza("[cFechaSancion]","");
				rep.comRemplaza("[cOtorgoSan]","");
			}
			...
			//Titulo Anterior.
			System.out.println("vRegs Titulo Anterior " + vRegsTituloAnterior.size());
			if (vRegsTituloAnterior.size()>0){
				for(int i=0;i<vRegsTituloAnterior.size();i++){
					...
					System.out.println("\n\n\n\n\n\n :::::::::::::::::::::::::::. \n\n Fecha del DOF: " + vTituloAnt.getDate("dtPublicacionDOF"));
			...
			System.out.println("Valor de la Fecha de Concesión: " + dtConcesion);
		...
		System.out.println("Valor del iEjercicio: " + iEjercicio);
		System.out.println("Valor del iMovProcedimiento" + iMovProcedimiento);
		System.out.println("Valor del iEjercicioOficio" + iEjercicioOficio);
		System.out.println("Valor del iEjercicioOficio" + iEjercicioOficio);
