...
public class TDigitoVerificador {
  ...
  public void setCDigito(String cCadenaOrigen) {
	...
	/*if(lCadenaValida == null)
		System.out.println("Cadena de matrícula no válida: " + cCadenaOrigen);
	else {*/
	System.out.println(">>>  Cadena Original:   "+cCadenaOrigen);
		for(int i=0; i<cCadenaOrigen.length(); i++) {
			...
			System.out.println(">>>  VALOR DE SERIE: "+iValorAct);
			
			iAcumProd += iValorAct * (iValorAct + 1);
		}
		//System.out.println("iAcumProd=" + iAcumProd);
		iRecalculo = (iAcumProd * iAcumProd) + (2*(100 * iAcumProd)) + (100 * 100);
		//System.out.println("iRecalculo=" + iRecalculo);
		cSubCadena = (new Integer(iRecalculo)).toString().substring(1);
		System.out.println("cSubCadena=" + cSubCadena);
		while (iSubProducto >= 10) {
			...
			//System.out.println("iSubProducto=" + iSubProducto);
			cSubCadena = new Integer(iSubProducto).toString();
		}
		this.iDigito = new Integer(iSubProducto).intValue();
		//System.out.println("Matrícula completa: " + cCadenaOrigen + "-" + this.iDigito);
	//}
	//return iDigito;
  }
...
//System.out.println(valores);
    if(valores != null && valores.length() > 7 ){
      ...
      for(int i = 0; i < dIni.length;i++){
        ...
        if(iFinal[i] > 9)
          iFinal[i] = iFinal[i] - 10;
//        System.out.println(iFinal[i]+" - "+dIni[i]);
      }
...
//      System.out.println("iDPreVerif : "+iDPreVerif);
...
//      System.out.println("iDigitoverificador : "+iDigitoverificador);
